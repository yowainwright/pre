...
e2e_pause() {
  printf '\n'
  [ "${PRE_E2E_FAST:-}" = "1" ] && return
  sleep "$E2E_STEP_DELAY"
}

e2e_start() {
  local manager="${1:?}"
  # The profile is generated by pre setup in the test container.
...
e2e_finish() {
  ...
  [ "${PRE_E2E_NONINTERACTIVE:-}" = "1" ] && return
  exec bash -i
}
