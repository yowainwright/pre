...
test_hook_paths() {
  ...
  # agent hook paths
  tmp_dir="$(mktemp -d)"
  check "agent_lint_hook_path" "$tmp_dir/scripts/agent/lint.sh" "$(agent_lint_hook_path "$tmp_dir")"
...
test_hook_installed() {
  tmp_dir="$(mktemp -d)"
  check "hook_installed rejects missing file" "1" "$(exit_code hook_installed "$tmp_dir/pre-commit")"
  touch "$tmp_dir/pre-commit"
  check "hook_installed accepts existing file"  "0" "$(exit_code hook_installed "$tmp_dir/pre-commit")"
  rm -rf "$tmp_dir"
}
...
test_check_agent_hooks() {
  tmp_dir="$(mktemp -d)"
  mkdir -p "$tmp_dir/scripts/agent"
  printf '#!/usr/bin/env sh\n' > "$tmp_dir/scripts/agent/lint.sh"
  chmod +x "$tmp_dir/scripts/agent/lint.sh"
  check_agent_hooks "$tmp_dir" >/dev/null
  check "check_agent_hooks installs Codex hooks" "0" "$(exit_code grep -q "scripts/agent/lint.sh" "$tmp_dir/.codex/hooks.json")"
  check "check_agent_hooks installs Claude settings" "0" "$(exit_code grep -q "scripts/agent/lint.sh" "$tmp_dir/.claude/settings.json")"
  rm -rf "$tmp_dir"
}
...
test_codex_merge() {
  ...
  check "install_codex_agent_hook adds lint hook" "0" "$(exit_code grep -q "scripts/agent/lint.sh" "$tmp_dir/.codex/hooks.json")"
  rm -rf "$tmp_dir"
}
...
test_claude_merge() {
  ...
  check "install_claude_agent_hook adds lint hook" "0" "$(exit_code grep -q "scripts/agent/lint.sh" "$tmp_dir/.claude/settings.json")"
  rm -rf "$tmp_dir"
}
